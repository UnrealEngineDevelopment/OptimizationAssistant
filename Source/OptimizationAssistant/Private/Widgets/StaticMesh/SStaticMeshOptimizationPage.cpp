...
void SStaticMeshOptimizationPage::ProcessOptimizationCheck(UStaticMesh* StaticMesh, FOutputDevice& Ar)
{
	if (StaticMesh && StaticMesh->RenderData && RuleSettings)
	{
		...
		{
			FString ErrorMessage;
			CheckTrianglesLODNum(StaticMesh, ErrorMessage);
			CheckLODNumLimit(StaticMesh, ErrorMessage);
			CheckLODTrianglesLimit(StaticMesh, ErrorMessage);
			CheckLODScreenSizeLimit(StaticMesh, ErrorMessage);
			CheckLODUVChannelLimit(StaticMesh, ErrorMessage);
			CheckLODMaterialNumLimit(StaticMesh, ErrorMessage);
			CheckLODDuplicateMaterials(StaticMesh, ErrorMessage);
			CheckMeshMaterialNumLimit(StaticMesh, ErrorMessage);
			if (!ErrorMessage.IsEmpty())
			{
...
void SStaticMeshOptimizationPage::CheckTrianglesLODNum(UStaticMesh * StaticMesh, FString & ErrorMessage)
{
	UGlobalCheckSettings* GlobalCheckSettings = GetMutableDefault<UGlobalCheckSettings>();
	if (!GlobalCheckSettings->HasAnyFlags(EOptimizationCheckFlags::OCF_TrianglesLODNum)) return;

	int32 NumLODs = StaticMesh->GetNumLODs();
	int32 MaxTriangles = StaticMesh->RenderData->LODResources[0].GetNumTriangles();
...
void SStaticMeshOptimizationPage::CheckLODNumLimit(UStaticMesh * StaticMesh, FString & ErrorMessage)
{
	UGlobalCheckSettings* GlobalCheckSettings = GetMutableDefault<UGlobalCheckSettings>();
...
void SStaticMeshOptimizationPage::CheckLODTrianglesLimit(UStaticMesh * StaticMesh, FString & ErrorMessage)
{
	UGlobalCheckSettings* GlobalCheckSettings = GetMutableDefault<UGlobalCheckSettings>();
	if (!GlobalCheckSettings->HasAnyFlags(EOptimizationCheckFlags::OCF_LODTrianglesLimit)) return;

	int32 NumLODLevels = StaticMesh->RenderData->LODResources.Num();
	int32 MaxTriangles = StaticMesh->RenderData->LODResources[0].GetNumTriangles();
	for (int32 LODIndex = 0; LODIndex < NumLODLevels; ++LODIndex)
	{
		...
		int32 LODTriangles = LODModel.GetNumTriangles();
...
void SStaticMeshOptimizationPage::CheckLODScreenSizeLimit(UStaticMesh * StaticMesh, FString & ErrorMessage)
{
	UGlobalCheckSettings* GlobalCheckSettings = GetMutableDefault<UGlobalCheckSettings>();
	...
	for (int32 LODIndex = 0; LODIndex < NumLODLevels; ++LODIndex)
	{
		float LODScreenSize = StaticMesh->RenderData->ScreenSize[LODIndex].GetValue();
		if (const PlatformInfo::FPlatformInfo* TargetPlatform = FOptimizationAssistantHelpers::GetTargetPlatform())
		{
			LODScreenSize = StaticMesh->RenderData->ScreenSize[LODIndex].GetValueForPlatformIdentifiers(TargetPlatform->PlatformGroupName, TargetPlatform->VanillaPlatformName);
		}
...
void SStaticMeshOptimizationPage::CheckLODUVChannelLimit(UStaticMesh * StaticMesh, FString & ErrorMessage)
{
	UGlobalCheckSettings* GlobalCheckSettings = GetMutableDefault<UGlobalCheckSettings>();
	...
	{
		FStaticMeshLODResources& LODModel = StaticMesh->RenderData->LODResources[LODIndex];
		int32 UVChannels = LODModel.VertexBuffers.StaticMeshVertexBuffer.GetNumTexCoords();
...
void SStaticMeshOptimizationPage::CheckLODMaterialNumLimit(UStaticMesh * StaticMesh, FString & ErrorMessage)
{
	UGlobalCheckSettings* GlobalCheckSettings = GetMutableDefault<UGlobalCheckSettings>();
...
void SStaticMeshOptimizationPage::CheckLODDuplicateMaterials(UStaticMesh * StaticMesh, FString & ErrorMessage)
{
	UGlobalCheckSettings* GlobalCheckSettings = GetMutableDefault<UGlobalCheckSettings>();
...
void SStaticMeshOptimizationPage::CheckMeshMaterialNumLimit(UStaticMesh * StaticMesh, FString & ErrorMessage)
{
	UGlobalCheckSettings* GlobalCheckSettings = GetMutableDefault<UGlobalCheckSettings>();
